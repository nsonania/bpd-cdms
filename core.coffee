	...
	db.Course.findOne titles: $elemMatch: compcode: Number compcode, (err, course) ->
		db.Student.find validated: true, selectedcourses: $elemMatch: compcode: $in: course.get("titles").map((x) -> Number x.compcode), (err, students) ->
			...
			for section in course.get("lectureSections") ? []
				str += "Lecture Section: #{section.number}, Instructor: #{section.instructor}, Enrolled: #{students._filter((x) -> x.get("selectedcourses")._any (y) -> y.selectedLectureSection is section.number).length}\n"
				str += "Student Id, Student Name\n"
			...
			for section in course.get("labSections") ? []
				str += "Lab Section: #{section.number}, Instructor: #{section.instructor}, Enrolled: #{students._filter((x) -> x.get("selectedcourses")._any (y) -> y.selectedLabSection is section.number).length}\n"
				str += "Student Id, Student Name\n"
