...
(function() {
  ...
  $(document).ready(function() {
    ...
    $("#addcourse_button").click(function() {
      var cid, course, crcs, i, section, slot, slots, tcourse, _i, _j, _k, _len, _len1, _len2, _ref;
      cid = $("#newcourse_select").children(":selected").attr("nbr").split("_").join(" ");
      tcourse = selectedCourse = course = _(courses).select(function(x) {
        return x.number === cid;
      })[0];
      ...
      $("#selcourses td button").last().click(function() {
        var crcs, i, section, _j, _k, _l, _len1, _len2, _len3, _ref;
        ...
        $("#newcourse_select").html("");
        crcs = _(courses).select(function(x) {
          return !($("#selcourses").text().match(x.number) != null);
        });
        for (i = _k = 0, _len2 = crcs.length; _k < _len2; i = ++_k) {
          course = crcs[i];
          $("#newcourse_select").append("<option nbr='" + (course.number.split(" ").join("_")) + "'>" + course.number + ": " + course.name + "</option>");
        }
        $("#section_select").html("");
      ...
      for (i = _j = 0, _len1 = crcs.length; _j < _len1; i = ++_j) {
        course = crcs[i];
        $("#newcourse_select").append("<option nbr='" + (course.number.split(" ").join("_")) + "'>" + course.number + ": " + course.name + "</option>");
      }
      $("#section_select").html("");
